...
# ydata = ([-1.,1.,-1.,-1.,-1.])
# print()
lengthM = int(len(arr[(len(arr)-1)].split(',')))
# lengthM = 9
ydata = np.arange(lengthM, dtype=np.float_).reshape(lengthM,1)

q = 0;
for z in arr[(len(arr)-1)].split(','):
    ydata[q] = z
    q = q+1
...
#
z = 0
K = np.arange((lengthM*lengthM), dtype=np.float_).reshape(lengthM,lengthM)
#
for x in arr:
    m = 0
    for y in x.split(','):
...
for tampili in range(banyakdtalpha):
    tampili+1,len(str(alphas[tampili][0]))
...
for tampilai in range(banyakdtalpha):
    tampilai+1
    # print ("nilai alpha",tampilai+1,"adalah","{:.15f}".format(np.round(alphas[tampilai][0], pdalpha)))
    print ("{:.8f}".format(np.round(alphas[tampilai][0], pdalpha)),",")
...
# print(w)
print("{:.8f}".format(round(bias, pdalpha)))
# print(bias)
# print(K[cond])
# print(w)
